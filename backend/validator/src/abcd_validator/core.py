...
def worker_validate(task: str | tuple[str, str]) -> ValidationResult:
    if isinstance(task, str):
        ...
        try:
            ...
            return ValidationResult(
                ...
                errors=[
                    ValidationError(
                        message=f"Failed to read from ZIP: {e}",
                        error_type="processing",
                        severity="error",
                    )
                ],
    ...
    return ABCDValidator._validate_single_file_static(
        content, filename, WORKER_SCHEMA_PATH, WORKER_RULES_FILE
    )
...
class ProgressDisplay:
    ...
    def update_download(self, downloaded: int, total: int):
        ...
        print(
            f"\rDownloading: {percent:.1f}% ({downloaded}/{total} bytes) - ETA: {eta:.1f}s",
            end="",
            flush=True,
        )
    ...
    def update_processing(self, processed: int, total: int):
        ...
        print(
            f"\rProcessing: {percent:.1f}% ({processed}/{total} files) - ETA: {eta:.1f}s",
            end="",
            flush=True,
        )
...
class ABCDValidator:
    ...
    def __init__(self, schema_path: str = None, rules_file: str | None = None):
            ...
            try:
                ...
                if schema_file:
                    self.schema_path = os.path.join(
                        os.path.dirname(__file__), "schemas", schema_file
                    )
                    logging.info(f"Detected and set schema path to {self.schema_path}")
            except Exception as e:
                logging.warning(f"Schema detection failed: {e}")

    def _detect_schema_version(self, tree: etree.ElementTree) -> str | None:
        try:
            root = tree.getroot()
            nsmap = root.nsmap
        ...
        try:
            ...
            if "<?xml" in decoded[:100]:
                ...
                if encoding_match and encoding_match.group(1).lower() != "utf-8":
                    return (
                        False,
                        f"XML declares encoding as {encoding_match.group(1)} but should be UTF-8",
                    )
            return True, None
        except UnicodeDecodeError as e:
            pos = e.start
            context = xml_content[max(0, pos - 20) : min(len(xml_content), pos + 20)].decode(
                "utf-8", errors="replace"
            )
            return False, f"Invalid UTF-8 encoding at position {pos}. Context: ...{context}..."

    @staticmethod
    def _validate_single_file_static(
        xml_content: bytes, filename: str, schema_path: str = None, rules_file: str | None = None
    ) -> ValidationResult:
        ...
        if not is_valid_utf8:
            return ValidationResult(
                ...
                errors=[
                    ValidationError(
                        message=f"Encoding error: {encoding_error}",
                        error_type="encoding",
                        severity="error",
                    )
                ],
        ...
        try:
            ...
            return ValidationResult(
                ...
                errors=[
                    ValidationError(
                        message=f"Unexpected error during parsing: {e}",
                        error_type="unknown",
                        severity="error",
                    )
                ],
        ...
        if validator.schema is None and validator.schema_path:
            try:
                ...
                errors.append(
                    ValidationError(
                        message=f"Schema loading failed: {e}",
                        error_type="processing",
                        severity="error",
                    )
                )
            ...
            try:
                ...
                if not schema_valid:
                    for error in validator.schema.error_log:
                        path = error.path or (
                            etree.ElementTree(doc).getpath(error.node)
        ...
        if validator.custom_validator:
            try:
                ...
                errors.append(
                    ValidationError(
                        message=f"Custom rules validation failed: {e}",
                        error_type="unknown",
                        severity="error",
                    )
                )
    ...
    def process_archive(
        ...
        with zipfile.ZipFile(archive_path) as zf:
            ...
            with ProcessPoolExecutor(
                ...
                initargs=(self.schema_path, self.rules_file),
            ) as executor:
...
def validate_archive(
    ...
    progress: ProgressDisplay = None,
    file_batch_size: int = DEFAULT_BATCH_SIZE,
    workers: int = mp.cpu_count(),
) -> Iterator[ValidationResult]:
    ...
    if os.path.isdir(archive_path):
        ...
        return validator.process_files(
            xml_files, progress=progress, batch_size=file_batch_size, workers=workers
        )
    elif archive_path.lower().endswith(".zip"):
        return validator.process_archive(
            archive_path, progress=progress, batch_size=file_batch_size, workers=workers
        )
    elif archive_path.lower().endswith(".xml"):
        xml_files = (
            glob.glob(archive_path)
            if ("*" in archive_path or "?" in archive_path)
            else [archive_path]
        )
        return validator.process_files(
            xml_files, progress=progress, batch_size=file_batch_size, workers=workers
...
def main():
    ...
    parser.add_argument(
        "--batch-files", type=int, default=100, help="Batch size for file processing tasks"
    )
    parser.add_argument(
        "--batch-aggregate", type=int, default=50, help="Batch size for aggregator updates"
    )
    parser.add_argument(
        "--batch-report", type=int, default=100, help="Batch size for report generation"
    )
    # New parameters
    parser.add_argument(
        "--download-timeout", type=float, default=30.0, help="Timeout for URL downloads in seconds"
    )
    parser.add_argument(
        "--download-retries", type=int, default=5, help="Number of download retries"
    )
    parser.add_argument(
        "--download-backoff", type=float, default=0.5, help="Backoff factor for download retries"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--memory-limit", type=int, help="Memory limit in MB to auto-tune batch sizes"
    )
    ...
    if args.memory_limit:
        ...
        logging.info(
            f"Adjusted batch-files to {batch_files}, batch-report to {batch_report} based on memory limit {args.memory_limit}MB"
        )
