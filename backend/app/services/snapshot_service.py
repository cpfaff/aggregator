...
class SnapshotService:
    ...
    def _calculate_validation_success_rate(self, days: int = 30) -> Optional[float]:
        ...
        completed_jobs = self.db.query(ValidationJobModel).filter(
            and_(
                ValidationJobModel.created_at >= cutoff_date,
    ...
    def get_dataset_unit_count(self, dataset_id: int) -> int:
        ...
        result = self.db.query(
            func.sum(ArchiveSnapshotModel.unit_count)
        ).join(
            latest_per_archive,
            and_(
    ...
    def get_biological_units_timeline(
        ...
        dates_query = self.db.query(
            func.date(ArchiveSnapshotModel.recorded_at).label('date')
        ).distinct().order_by(
            desc(func.date(ArchiveSnapshotModel.recorded_at))
        )
    ...
    def get_provider_biological_units_timeline(
        ...
        provider_archive_ids = [
            r[0] for r in self.db.query(XmlArchiveModel.id).join(
                DatasetModel, XmlArchiveModel.dataset_id == DatasetModel.id
            ).filter(
                DatasetModel.provider_id == provider_id,
                XmlArchiveModel.isLatest == True
    ...
    def get_provider_datasets_timeline(
        ...
        if period == "monthly":
            dataset_results = self.db.query(
                ...
                func.date_trunc('month', DatasetModel.created_at)
            ).order_by(
                func.date_trunc('month', DatasetModel.created_at)
            ).all()

            # Get baseline count (datasets before cutoff)
            baseline_count = self.db.query(func.count(DatasetModel.id)).filter(
                DatasetModel.provider_id == provider_id,
                DatasetModel.created_at < cutoff_date
            ).scalar() or 0
            ...
            dataset_results = self.db.query(
                ...
                func.date(DatasetModel.created_at)
            ).order_by(
                func.date(DatasetModel.created_at)
            ).all()
    ...
    def get_multi_provider_timeline(
        ...
        providers = self.db.query(
            ...
            XmlArchiveModel, DatasetModel.id == XmlArchiveModel.dataset_id
        ).join(
            ArchiveSnapshotModel, XmlArchiveModel.id == ArchiveSnapshotModel.archive_id
        ).filter(
            XmlArchiveModel.isLatest == True
        ).distinct().all()
        ...
        provider_metadata = [
            {'id': p_id, 'name': p_name, 'key': p_name}
    ...
    def get_provider_statistics(
        ...
        archive_count = self.db.query(func.count(XmlArchiveModel.id)).join(
            DatasetModel
        ).filter(
            DatasetModel.provider_id == provider_id
        ).scalar() or 0
        ...
        # Get last activity
        last_dataset = self.db.query(DatasetModel).filter(
            DatasetModel.provider_id == provider_id
        ).order_by(desc(DatasetModel.updated_at)).first()
    ...
    def _calculate_provider_validation_rate(self, provider_id: int) -> Optional[float]:
        ...
        completed_jobs = self.db.query(ValidationJobModel).join(
    ...
    def get_dataset_statistics(
        ...
        if latest_validation:
            ...
            is_valid = (
                latest_validation.valid_files == latest_validation.total_files and
                latest_validation.total_files > 0
            ) if latest_validation.status == 'completed' else None
    ...
    def get_provider_list_stats(
        ...
        datacenter_stats = self.db.query(
            ...
            DatasetModel, DatasetModel.provider_id == DataProviderModel.id
        ).filter(
    ...
    def get_recent_activity(
        ...
        recent_datasets = self.db.query(DatasetModel).filter(
            DatasetModel.created_at >= cutoff_date
        ).order_by(desc(DatasetModel.created_at)).limit(limit).all()
    ...
    def get_health_status(
        ...
        recent_validations = self.db.query(func.count(ValidationJobModel.id)).filter(
            ValidationJobModel.created_at >= cutoff
        ).scalar() or 0
    ...
    def get_growth_metrics(
        ...
        providers_timeline = []
        if period == "monthly":
            provider_results = self.db.query(
                func.date_trunc('month', DataProviderModel.created_at).label('month'),
                func.count(DataProviderModel.id).label('count')
            ).filter(
                DataProviderModel.created_at >= cutoff_date
            ).group_by(
                func.date_trunc('month', DataProviderModel.created_at)
            ).order_by(
                func.date_trunc('month', DataProviderModel.created_at)
            ).all()

            baseline_count = self.db.query(func.count(DataProviderModel.id)).filter(
                DataProviderModel.created_at < cutoff_date
            ).scalar() or 0
            ...
            provider_results = self.db.query(
                ...
                func.date(DataProviderModel.created_at)
            ).order_by(
                func.date(DataProviderModel.created_at)
            ).all()
        ...
        if period == "monthly":
            ...
            validation_timeline = [
                ...
                for row in validation_results if row.month
            ]
        else:
            validation_results = self.db.query(
                ...
                func.date(ValidationJobModel.created_at)
            ).order_by(
                func.date(ValidationJobModel.created_at)
            ).all()

            validation_timeline = [
                {"date": row.day, "value": row.count}
                for row in validation_results if row.day
            ]
