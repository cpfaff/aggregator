...
def get_stats_worker_concurrency(
    concurrency_setting: Union[str, int],
    validator_cpus: int = None
) -> int:
    """
    ...
    if isinstance(concurrency_setting, int):
        if concurrency_setting < 2:
            logger.warning(
                f"Stats concurrency {concurrency_setting} below minimum, using 2"
            )
            return 2
    ...
    logger.warning(
        f"Invalid concurrency setting '{concurrency_setting}', using auto mode"
    )
