...
def list_tasks(show_all: bool = False):
        ...
        for worker, tasks in active.items():
            ...
            if tasks:
                ...
                for task in tasks:
                    ...
                    print(f"  [{task_count}] {task_name:<30} | ID: {task['id'][:8]}... | ⏱️  {duration}")
                    
                    # Show relevant kwargs
                    if 'kwargs' in task and task['kwargs']:
                        params = []
                        for k, v in task['kwargs'].items():
                            if k in ['batch_size', 'offset', 'dataset_id', 'provider_id', 'target_date']:
                                params.append(f"{k}={v}")
                        if params:
    ...
    if validation_worker_name not in found_workers:
        ...
        try:
            ...
            if result.stdout and "received" in result.stdout or "Starting validation" in result.stdout:
        ...
        if scheduled:
            ...
            for worker, tasks in scheduled.items():
                if tasks:
                    ...
                    for task in tasks:
                        scheduled_count += 1
                        task_name = task['request']['name'].replace('statistics.', '').replace('validator.', '')
        ...
        for worker, info in stats.items():
            if info:
                ...
                if 'VALIDATION' in worker_type:
                    completed = sum(v for k, v in total.items() if k.startswith('tasks.validator.') and isinstance(v, (int, float)))
                else:
                    completed = sum(v for k, v in total.items() if k.startswith('tasks.statistics.') and isinstance(v, (int, float)))
...
def main():
    ...
    parser.add_argument('--list-all', action='store_true', help='List all tasks (running, scheduled, queued)')
    ...
    parser.add_argument('--force', action='store_true', help='Force terminate tasks (use with --cancel or --cancel-all)')
