    ...
    async with async_session() as db:
        try:
            ...
            result = await db.execute(
                select(DatasetModel).where(DatasetModel.title.like("%test%")).limit(1)
            )
            ...
            if not test_dataset:
                ...
                provider_result = await db.execute(select(DataProviderModel).limit(1))
                ...
                xml_archive = XmlArchiveModel(
                    dataset_id=test_dataset.id, url="https://example.com/test.xml", isLatest=True
                )
                db.add(xml_archive)
                await db.flush()
                ...
                validation_job = ValidationJobModel(
                    archive_id=xml_archive.id, created_at=datetime.now(), updated_at=datetime.now()
                )
            ...
            stats_count_result = await db.execute(
                select(func.count(StatisticModel.id)).where(
            ...
            archives_count_result = await db.execute(
                select(func.count(XmlArchiveModel.id)).where(
            ...
            if archive_ids:
                validation_count_result = await db.execute(
                    select(func.count(ValidationJobModel.id)).where(
            ...
            links_count_result = await db.execute(
                select(func.count(UsefulLinkModel.id)).where(
