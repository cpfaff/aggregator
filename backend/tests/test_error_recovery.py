...
class TestFailureTracking:
    ...
    def test_mark_archive_failed_increment_count(
        self, mock_db_session, sample_statistic_with_failure
    ):
...
class TestArchiveProcessingState:
    ...
    def test_is_archive_first_processing_with_failure(
        self, mock_db_session, sample_statistic_with_failure
    ):
...
class TestTimeBasedReset:
    ...
    def test_should_retry_under_max_retries(self, mock_db_session):
        ...
        failure_info = {"failure_count": 3, "last_failure_at": datetime.utcnow().isoformat()}
        with patch(
            "app.tasks.statistics_tasks.get_archive_failure_info", return_value=failure_info
        ):
    ...
    def test_should_not_retry_exceeded_max_recent_failure(self, mock_db_session):
        ...
        with patch(
            ...
            result = should_retry_failed_archive(
                mock_db_session, 123, max_retries=5, reset_after_hours=24
            )
    ...
    def test_should_retry_on_invalid_timestamp(self, mock_db_session):
        ...
        failure_info = {"failure_count": 5, "last_failure_at": "invalid-timestamp"}
        with patch(
            "app.tasks.statistics_tasks.get_archive_failure_info", return_value=failure_info
        ):
...
class TestCircuitBreaker:
    ...
    def test_skip_archives_exceeding_max_retries(
        self, mock_parse, mock_session_local, mock_db_session
    ):
        ...
        with patch(
            "app.tasks.statistics_tasks.should_retry_failed_archive",
            side_effect=should_retry_side_effect,
        ):
            with patch("app.tasks.statistics_tasks.get_archive_failure_info") as mock_failure_info:
...
class TestTimelineIntegrity:
    ...
    def test_failed_archives_dont_corrupt_timeline(self, mock_session_local, mock_db_session):
        ...
        successful_stat = Mock(value=100, date=date(2024, 1, 1), extra_data={"archive_id": 1})

        failed_stat = Mock(
            value=0,
            date=date(1970, 1, 1),  # Failure marker date
            extra_data={"archive_id": 2, "processing_failed": True, "failure_count": 3},
        )
...
class TestErrorLogging:
    ...
    def test_error_logging_includes_context(
        self, mock_parse, mock_session_local, mock_logger, mock_db_session
    ):
