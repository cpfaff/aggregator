...
def test_worker_memory_limits():
    ...
    for line in config_lines:
        if "celery_worker_validation:" in line:
            ...
            in_validation_worker = False
        elif line.strip().startswith("celery_") or line.strip().startswith("frontend:") or line.strip().startswith("backend:"):
            in_validation_worker = False
            in_stats_worker = False
            
        # Check for memory limits (2G = 2147483648 bytes, 1G = 1073741824 bytes)
        if in_validation_worker and ("memory: 2G" in line or "memory: \"2147483648\"" in line or "2147483648" in line):
            validation_memory_found = True
        elif in_stats_worker and ("memory: 1G" in line or "memory: \"1073741824\"" in line or "1073741824" in line):
        ...
        if "celery-beat:" in line:
            ...
            for j in range(i, min(i+15, len(lines))):
                if "depends_on:" in lines[j]:
                    ...
                    deps_text = "\n".join(lines[j:j+10])
                    if "celery_worker_validation" in deps_text and "celery_worker_stats" in deps_text:
                        beat_deps_correct = True
                    break
            break
    
    assert beat_deps_correct, "celery-beat should depend on both celery_worker_validation and celery_worker_stats"
