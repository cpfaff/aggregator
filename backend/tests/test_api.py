...
from app.models import DataProviderModel, DatasetModel, UsefulLinkModel, UserModel, XmlArchiveModel
...
async def admin_token(admin_user):
    ...
    return create_access_token(
        data={"sub": admin_user.username, "is_admin": True}, expires_delta=access_token_expires
    )
...
def test_expired_token(client, test_user):
    ...
    resp = client.get(
        f"/users/{test_user.username}", headers={"Authorization": f"Bearer {expired_token}"}
    )
    assert resp.status_code == 401
...
def test_unauthorized_user_creation(client, test_user_token):
    ...
    unique_suffix = uuid.uuid4().hex[:8]
    new_user = {
        "username": f"newuser2_{unique_suffix}",
        "password": "secure123",
        "is_global_admin": False,
    }
    resp = client.post(
        "/users", json=new_user, headers={"Authorization": f"Bearer {test_user_token}"}
    )
...
def test_create_data_provider(client, admin_token):
    ...
    resp = client.post(
        "/data-providers", json=new_provider, headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_create_useful_link(client, admin_token, test_provider, test_dataset):
    ...
    unique_suffix = uuid.uuid4().hex[:8]
    new_link = {
        "title": f"New Link {unique_suffix}",
        "url": f"http://example.com/new_{unique_suffix}",
        "isLatest": True,
    }
    resp = client.post(
...
def test_delete_user(client, admin_token, db_session):
    ...
    resp = client.delete(
        f"/users/{user_to_delete.username}", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 204


def test_harvest_datasets(
    client, test_provider, test_dataset, test_xml_archive, test_useful_link, admin_token
):
...
def test_get_user(client, admin_token, test_user):
    ...
    resp = client.get(
        f"/users/{test_user.username}", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 200
    assert resp.json()["username"] == test_user.username

    # Test non-existent user
    resp = client.get("/users/nonexistentuser", headers={"Authorization": f"Bearer {admin_token}"})
...
def test_invalid_token_format(client):
    ...
    resp = client.get("/users", headers={"Authorization": "Bearer invalid.token.format"})
    assert resp.status_code == 401
    assert "could not validate credentials" in resp.json()["detail"].lower()

    # Try with a token that's not even a Bearer token
    resp = client.get("/users", headers={"Authorization": "NotBearer some_token"})
    assert resp.status_code == 401
...
def test_get_provider(client, admin_token, test_provider):
    ...
    resp = client.get(
        f"/data-providers/{test_provider.id}", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 200
    ...
    resp = client.get("/data-providers/999999", headers={"Authorization": f"Bearer {admin_token}"})
    assert resp.status_code == 404
...
def test_delete_provider(client, admin_token, test_provider):
    ...
    # Create the provider
    resp = client.post(
        "/data-providers", headers={"Authorization": f"Bearer {admin_token}"}, json=provider_data
    )
    assert resp.status_code == 201
    ...
    resp = client.delete(
        f"/data-providers/{provider_id}", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 204
    ...
    resp = client.delete(
        "/data-providers/999999", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 204
...
def test_user_provider_permissions(client, admin_token, test_provider):
    ...
    resp = client.post("/users", headers={"Authorization": f"Bearer {admin_token}"}, json=user_data)
    assert resp.status_code == 201
    ...
    resp = client.get(
        f"/data-providers/{test_provider.id}", headers={"Authorization": f"Bearer {user_token}"}
    )
    assert resp.status_code == 200
    assert resp.json()["id"] == test_provider.id
...
def test_get_datasets(client, admin_token, test_provider, test_dataset):
    ...
    resp = client.get(
        "/data-providers/9999/data-sets", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 200
    assert resp.json() == []
...
def test_update_dataset(client, admin_token, test_provider, test_dataset):
    ...
    updated_data_with_archives = {
        ...
        "xmlArchives": [{"url": "http://example.com/new-archive.xml", "isLatest": True}],
    }
    ...
    assert any(
        archive["url"] == "http://example.com/new-archive.xml" for archive in dataset["xmlArchives"]
    )
    ...
    updated_data_with_links = {
        ...
        "usefulLinks": [
            {"title": "New Useful Link", "url": "http://example.com/new-link", "isLatest": True}
        ],
    }
...
def test_xml_archive_permissions(client, admin_token, test_provider, test_dataset):
    ...
    xml_archive_data = {"url": "http://example.com/test-archive.xml", "isLatest": True}
...
def test_get_useful_links(client, admin_token, test_provider, test_dataset, db_session):
    ...
    useful_link1 = {"title": "Test Link 1", "url": "http://example.com/link1", "isLatest": True}

    useful_link2 = {"title": "Test Link 2", "url": "http://example.com/link2", "isLatest": False}
...
def test_nonexistent_resources(client, admin_token, test_provider, test_dataset):
    ...
    # Test non-existent provider
    resp = client.get(
        "/data-providers/999/data-sets", headers={"Authorization": f"Bearer {admin_token}"}
    )
    ...
    update_data = {"title": "Updated Title", "description": "Updated Description", "isLatest": True}
    resp = client.put(
        f"/data-providers/{test_provider.id}/data-sets/999",
        headers={"Authorization": f"Bearer {admin_token}"},
        json=update_data,
...
def test_permission_denied_scenarios(client, admin_token, test_provider, test_dataset):
    ...
    resp = client.post(
        "/users", json=no_role_user_data, headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert resp.status_code == 201
    ...
    resp = client.get("/data-providers", headers={"Authorization": f"Bearer {no_role_token}"})
    assert resp.status_code == 200

    # POST provider - should be denied for non-admin users
    provider_data = {"name": "Test Provider", "description": "Test Provider Description"}
    resp = client.post(
        "/data-providers", headers={"Authorization": f"Bearer {no_role_token}"}, json=provider_data
    )
    ...
    resp = client.delete(
        f"/data-providers/{test_provider.id}", headers={"Authorization": f"Bearer {no_role_token}"}
    )
    ...
    xml_archive_data = {"url": "http://example.com/xml-archive", "description": "Test XML Archive"}
    resp = client.post(
        f"/data-providers/{test_provider.id}/data-sets/{test_dataset.id}/xml-archives",
        headers={"Authorization": f"Bearer {no_role_token}"},
        json=xml_archive_data,
...
def test_delete_permission_checks():
    ...
    import pytest
    from fastapi import HTTPException
    from main import check_provider_permission
...
def test_authentication_detailed_error_handling():
    ...
    class MockUserModel:
        def __init__(self):
            ...
            salt = bcrypt.gensalt()
            self.hashed_password = bcrypt.hashpw(b"correct_password", salt)
...
def test_missing_resource_errors():
    ...
    response = client.put(
        "/data-sets/999999", json={"name": "Updated Dataset", "description": "Updated description"}
    )
    assert response.status_code == 404
    error_detail = response.json()
    assert "detail" in error_detail
