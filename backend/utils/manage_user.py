...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Manage users in the database - create new users or update existing ones."
    )
    parser.add_argument(
        "--username", "-u", help="Username for the user. If not provided, will be prompted"
    )
    parser.add_argument(
        "--password", "-p", help="Password for the user. If not provided, will be prompted"
    )
    parser.add_argument(
        "--database-url", "-d", help="Database URL. If not provided, will use environment variable"
    )
    parser.add_argument(
        "--global-admin",
        "-g",
...
def get_database_url(args):
    ...
    if not database_url:
        ...
        print(
            "Please provide a database URL using the --database-url option or set the DATABASE_URL environment variable"
        )
        sys.exit(1)
...
async def init_user(username, password, database_url, is_global_admin=True, force=False):
    ...
    async with async_session() as session:
        ...
        if user:
            ...
            if not force:
                confirm = input(
                    f"Do you want to update the existing user '{username}'? (y/n): "
                ).lower()
